...
// #include "../src/spectral/statistics.h"
...
void process_print (void)
{
  ...
  // actually a vector mean
  printf ( "spectral mean: %f", cvec_mean(fftgrain) );
  outmsg ("\t");
  printf ( "spectral centroid: %f\n", cvec_centroid(fftgrain) );
...
int main(int argc, char **argv) {
  ...
  // printf("norm[1]: %f ",  );
  // float sample = cvec_norm_get_sample(fftgrain[0)	

  // spectral_slope = fftgrain->norm;

  // printf("%d", fftgrain->norm->length)
  // spectral_slope = fftgrain->norm->length
